...
# print("Razorpay API Key:", os.getenv("RAZORPAY_KEY_ID"))
# print("Razorpay API Secret:", os.getenv("RAZORPAY_KEY_SECRET"))
...
def initiate_payment(request):
    if request.method == "GET":
        try:
            ...
            # Ensure safe Decimal conversions
            total_cost = sum(
                Decimal(item.quantity or 0) * Decimal(item.product.price or 0) for item in cart_items
            )
