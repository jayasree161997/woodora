...
def change_order_status(request, order_id, status):
    ...
    if order.status == "Cancelled":
        messages.error(request, "This order was already canceled. Status cannot be changed.")
        return redirect('custom_admin:order_management')

    # Handle return request process
    ...
    # Normal status update (Processing, Delivered, or Cancelled)
    if status in ["Processing", "Delivered", "Cancelled"]:
        order.status = status
        order.save()
...
def add_offer(request, product_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                offer.full_clean()  # this calls model's clean()
                offer.save()
                messages.success(request, f'Offer added to {product.name}.')
                return redirect('custom_admin:products_view')
            except ValidationError as e:
                form.add_error(None, e)  # attach general error
    else:
...
def generate_pdf(context):
    ...
    table_data = [["Product Name", "Quantity Sold"]]  # Table Header
...
def sales_report_view(request):
    ...
    print("Filtered Order Items Count:", order_items.count())

    product_counter = Counter()

    for item in order_items:
        product_counter[item.product] += item.quantity
        print("Product Counter Data:", product_counter)
