...
def product_details(request, product_id):
    ...
    related_products = Product.objects.filter(category=product.category).exclude(id=product.id)[:6]  # Show 4 related products
...
def cart_detail(request):
    ...
    total_cost = sum(item.quantity * item.product.price for item in updated_items)
