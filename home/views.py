...
def checkout(request):
    ...
    for item in cart_items:
        if not item.product.category.is_active:
            messages.error(request, f"Product '{item.product.name}' belongs to an inactive category and cannot be purchased.")
    ...
    # --------------- Offer logic corrected ----------------
    if total_price > 50000:
        # Above ₹50000: 30% Category Offer (applies normally on each item)
        for item in cart_items:
        ...
        # Between ₹20000 - ₹50000: ₹2000 Product Offer only once
        product_offer_applied = False  # 🔥 Very important flag

        for item in cart_items:
            if not product_offer_applied:  # 🔥 Apply offer only once
                ...
                if product_offers.exists():
                    discount_amount += 2000
                    product_offer_applied = True  # 🔥 After first time, stop applying
    # -------------------------------------------------------
...
def return_order(request, order_id):
    if request.method == "POST":
        try:
            ...
            print(f"Order ID: {order.id}, Current Status: {order.status}")  # Debugging
            ...
            # If order is eligible for return, mark it as 'Return Requested' instead of 'Return Completed'
...
def cancel_order(request, order_id):
    if request.method == "POST":
        ...
        if not order.razorpay_payment_id and payment_method not in ["cod", "cash on delivery"]:
            return JsonResponse({"error": "This order was not paid via Razorpay or is not a Cash on Delivery order."}, status=400)
...
def generate_invoice_data(order_id):
    ...
    if order.address:
        ...
        # If address is deleted, use backup fields stored inside Order
